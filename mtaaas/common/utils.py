...
from mtaaas.openstack.common import strutils
...
MTAAAS_TEST_SOCKET_FD_STR = 'MTAAAS_TEST_SOCKET_FD'
...
def add_features_to_http_headers(features, headers):
    """
    Adds additional headers representing mtaaas features to be enabled.
...
class PrettyTable(object):
    """Creates an ASCII art table for use in bin/mtaaas
    ...
    def __get_backend(self):
        if not self.__backend:
            ...
            if backend_name not in self.__backends:
                ...
                raise exception.MtaaasException(msg)
...
def get_test_suite_socket():
    global MTAAAS_TEST_SOCKET_FD_STR
    if MTAAAS_TEST_SOCKET_FD_STR in os.environ:
        fd = int(os.environ[MTAAAS_TEST_SOCKET_FD_STR])
        sock = socket.fromfd(fd, socket.AF_INET, socket.SOCK_STREAM)
        sock = socket.SocketType(_sock=sock)
        sock.listen(CONF.backlog)
        del os.environ[MTAAAS_TEST_SOCKET_FD_STR]
        os.close(fd)
        return sock
    return None
